...
const statusMap: { [key: string]: { label: string; color: string } } = {
  ...
  COMPLETED: { label: 'مكتمل', color: 'bg-emerald-800 text-emerald-50 border-emerald-700' },
...
function AppointmentsPageInner() {
  ...
  useEffect(() => {
    const fetchAppointments = async () => {
      ...
      try {
        setLoadingData(true)
        const response = await fetch('/api/student/appointments', { credentials: 'include' })
        const data: AppointmentsResponse = await response.json()
        if (!response.ok) { setError('فشل في جلب المواعيد'); return }
        if (data.success) {
  ...
  const handleOpenChat = async (caseId: string) => {
    try {
      const response = await fetch(`/api/cases/${caseId}/chat`, { method: 'POST', credentials: 'include' })
      const data = await response.json()
      if (!response.ok) throw new Error(data.error || 'فشل في فتح المحادثة')
  ...
  const formatDate = (dateString: string) => {
    ...
    if (date.toDateString() === tomorrow.toDateString()) return `غداً، ${date.toLocaleTimeString('ar-EG', { hour: '2-digit', minute: '2-digit' })}`
  ...
  const navUser = user ? {
    ...
    email: user.email || '',
    role: user.role as 'STUDENT' | 'PATIENT',
      ...
      <div className="min-h-screen flex flex-col">
        ...
        <main className="flex-1 py-8 px-4 bg-muted/30">
          <div className="container mx-auto max-w-5xl">
            ...
            <Button onClick={() => router.back()} variant="outline" className="mt-4">رجوع</Button>
          </div>
        </main>
        <Footer />
      </div>
  ...
  const now = new Date()
  const upcomingAppointments = appointments.filter(app => new Date(app.scheduledAt) > now)
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 py-8 px-4 bg-muted/30">
        <div className="container mx-auto max-w-5xl">
          <div className="mb-6">
            <Link href="/dashboard/student" className="text-sm text-muted-foreground hover:text-primary flex items-center gap-1 mb-4">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-muted-foreground mt-2" suppressHydrationWarning={true}>إدارة مواعيد العلاج مع المرضى</p>
              </div>
              {upcomingCount > 0 && (
              ...
              <Card>
                <CardContent className="py-12">
                  <div className="text-center">
                    ...
                    <p className="text-muted-foreground" suppressHydrationWarning={true}>سيظهر هنا المواعيد القادمة مع المرضى</p>
                  </div>
                </CardContent>
              </Card>
            ) : (
                  ...
                  <Card key={appointment.id} className="hover:shadow-md transition-shadow">
                    ...
                    <CardContent className="space-y-3">
                      ...
                      <div className="flex items-center gap-2 text-sm">
                        ...
                        <span className="text-muted-foreground">{appointment.location || appointment.caseAddress}</span>
                      </div>
                      {appointment.duration && (
                        <div className="flex items-center gap-2 text-sm">
                          <Clock className="w-4 h-4 text-muted-foreground" />
                          <span className="text-muted-foreground" suppressHydrationWarning={true}>المدة: {appointment.duration} دقيقة</span>
                        </div>
                      )}
                      <div className="flex gap-2 pt-2 border-t">
                        <Button onClick={() => handleOpenChat(appointment.caseId)} variant="outline" className="flex-1" suppressHydrationWarning={true}>
                          <MessageSquare className="w-4 h-4 ml-2" />
                          المحادثة
                        </Button>
                        <Button onClick={() => router.push(`/posts/${appointment.postId}`)} variant="outline" className="flex-1" suppressHydrationWarning={true}>
          ...
          {appointments.length > 0 && (
            <div>
              ...
              <div className="grid md:grid-cols-2 gap-4 opacity-60">
                {appointments.filter(app => new Date(app.scheduledAt) <= now).map((appointment) => (
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-3 mb-2">
                            ...
                            <div>
                              ...
                              <CardDescription className="text-gray-400">{appointment.postTitle}</CardDescription>
                            </div>
                          </div>
                          <div className="mt-2">{getStatusBadge(appointment.status)}</div>
                        </div>
